...
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,sans-serif;background:#f4f6fb;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:16px}.card{background:#fff;border-radius:14px;box-shadow:0 4px 24px rgba(0,0,0,.09);padding:32px;text-align:center;max-width:380px;width:100%}.logo{font-size:40px;margin-bottom:14px}h2{color:#D10039;font-size:18px;font-weight:800;margin-bottom:6px}p{color:#555;font-size:14px;margin-bottom:10px}.spinner{width:36px;height:36px;border:3px solid #f0f0f0;border-top-color:#D10039;border-radius:50%;animation:spin .8s linear infinite;margin:0 auto 14px}@keyframes spin{to{transform:rotate(360deg)}}.err{color:#b91c1c;font-weight:600}.dbg{background:#f8f8f8;border:1px solid #e0e0e0;border-radius:8px;padding:8px;font-size:10px;color:#777;white-space:pre-wrap;word-break:break-all;max-height:100px;overflow:auto;margin-bottom:12px} label{display:block;font-size:13px;font-weight:700;color:#333;margin-bottom:4px} input[type=number]{width:100%;border:1.5px solid #ccc;border-radius:8px;padding:10px;font-size:16px;margin-bottom:10px;outline:none} input[type=number]:focus{border-color:#D10039} .btn{width:100%;background:#D10039;color:#fff;border:none;border-radius:9px;padding:13px;font-size:15px;font-weight:700;cursor:pointer;margin-top:2px} .btn:disabled{background:#ccc;cursor:not-allowed} .status{font-size:12px;color:#555;text-align:center;margin-top:6px} .err{color:#b91c1c}</style>
</head>
<body><div class="card"><div class="logo">&#x1F4B3;</div><h2>HandyPay</h2><div class="dbg" id="dbg">Checking URL params...</div><label>Amount (JMD)</label><input type="number" id="amt" placeholder="e.g. 31000" min="80" step="1"><button class="btn" id="btn" onclick="payNow()">Pay with HandyPay</button><div class="status" id="status"></div></div>
<script>
var LOC="${ locId }",done=false;
function log(s){var e=document.getElementById('dbg');e.textContent+=s+'\n';}
...
function applyAmt(jmd){if(jmd>=80)document.getElementById('amt').value=jmd;}
function payNow(){
  if(done)return;
  var jmd=parseFloat(document.getElementById('amt').value)||0;
  if(jmd<80){setStatus('Min J$80 required',true);return;}
  done=true;document.getElementById('btn').disabled=true;setStatus('Creating session...');
  ...
  .then(function(d){if(d.checkoutUrl){setStatus('Redirecting...');window.location.href=d.checkoutUrl;}
    else{setStatus('Error: '+(d.error||'no URL'),true);done=false;document.getElementById('btn').disabled=false;}})
  .catch(function(e){setStatus('Network error',true);done=false;document.getElementById('btn').disabled=false;});
}
// 1. Check URL hash
var h=window.location.hash;log('URL: '+window.location.href);
if(h&&h.length>1){var hp={};h.slice(1).split('&').forEach(function(kv){var i=kv.indexOf('=');if(i>0)hp[decodeURIComponent(kv.slice(0,i))]=decodeURIComponent(kv.slice(i+1));});log('Hash: '+JSON.stringify(hp));var raw=hp.amount||hp.amountCents||0;if(raw>0)applyAmt(jmdFrom(raw,hp.currency));}
// 2. postMessage listener
window.addEventListener('message',function(e){
  var data=e.data||{};log('MSG['+e.origin+']: '+JSON.stringify(data).substring(0,200));
  ...
  var raw=data.amount||data.amountCents||data.total||0;var cur=data.currency||data.currencyCode||'';
...
// 3. Signal ready to GHL
try{window.parent.postMessage({type:'PAYMENT_PROVIDER_READY',provider:'handypay'},'*');window.parent.postMessage({action:'INIT'},'*');}catch(x){log('send err:'+x.message);}
</script></body></html>`;
    res.setHeader('Content-Type','text/html');
    return res.send(html);
  } catch(e) {
