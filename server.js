...
async function getFreshToken(locationId) {
  ...
  if (cfg.crm_refresh_token || cfg.ghl_refresh_token) {
    try { var fresh = await refreshCrmToken(locationId); if (fresh) tok = fresh; } catch(e){}
  } else {
    console.log('[getFreshToken] PIT mode for', locationId);
  }
  return tok;
}
...
app.post('/api/webhooks/crm', async (req, res) => {
  ...
  } else { console.log('[SMS] suppressed - test mode'); }
...
app.post('/api/webhooks/followup', async (req, res) => {
  ...
  if(depRow && fullRow) {
    ...
    try {
      ...
      var ds = await createHandyPaySession(config.handypay_api_key, depAmt, 'Deposit - '+title, Object.assign({},meta,{paymentType:'deposit'}));
...
app.all(['/api/query', '/api/query/:paymentIntentId'], async (req, res) => {
  ...
  try {
    ...
    console.log('[/api/query] paymentIntentId:', paymentIntentId);
    ...
    console.log('[/api/query] final status:', ghlStatus);
...
app.get('/api/init-db', async (req, res) => {
  ...
  try {
    await pool.query(`
      ...
      ALTER TABLE merchant_configs ADD COLUMN IF NOT EXISTS deposit_amount INTEGER DEFAULT 500;
  ...
  try {
    ...
    if (keyToUse && sessionIdToCheck) {
      try {
        ...
        console.error('[post-query] HP error:', hpErr.message);
      }
    }
