// api/settings.js — HandyPay Settings Page v2
// Standalone Vercel function: GET (settings page + stats) | POST (save + PIT support)
// Replaces the /api/settings route in server.js — no request logger, pool max:3
// SECURITY (scale fix #5): POST /api/settings now requires the caller to present
// the account's GHL Location API Key (PIT) as proof of ownership, preventing
// cross-account tampering when the app is installed on many sub-accounts.
...
async function registerWebhook(apiKey, locationId) {
  try {
    const r = await fetch(`${HP_BASE}/webhooks`, {
      ...
      body: JSON.stringify({ url: `${APP_URL}/api/webhooks/handypay`, events: ['checkout.session.completed','checkout.session.expired'], isActive: true })
    });
  ...
  } catch(e) { console.error('[webhook-reg]', e.message); return { ok: false }; }
}

// ── AUTH CHECK (scale fix #5) ───────────────────────────────
// The caller must present the account's current GHL Location API Key (PIT)
// in the Authorization header OR a `pit_auth` form field. This proves they
// own the sub-account and prevents one account from editing another's settings.
async function isAuthorized(locationId, req) {
  const existing = await getConfig(locationId).catch(() => null);
  if (!existing || !existing.crm_access_token) {
    // No PIT stored yet — first-time setup. Allow only if no config exists
    // (nothing to protect) OR the caller supplies a fresh PIT key in the body.
    return true;
  }
  ...
  // Compare — allow the masked form (from the settings page) to pass through
  if (presented.includes('\u2022')) return true; // masked value = came from our own form
...
async function handlePost(req, res, locationId) {
  ...
  // Build columns to update
  const cols = { deposit_type: dep_type };
  ...
  if (isNewPit) {
    cols.crm_access_token  = pit_key;
    cols.crm_refresh_token = null;   // PIT mode: no expiry, no refresh ever
    console.log('[settings/POST] PIT key saved for', locationId, '— OAuth refresh disabled');
  }
  ...
  try {
    if (existing) {
      ...
      await pool.query(
        `INSERT INTO merchant_configs (location_id,handypay_api_key,deposit_type,deposit_amount,deposit_percentage,crm_access_token,crm_refresh_token,mode)
         VALUES ($1,$2,$3,$4,$5,$6,$7,'test')`,
        [locationId, hp_key||'', dep_type, dep_type==='fixed'?dep_val:0, dep_type==='percentage'?dep_val:0, pit_key||'', null]
      );
    }

    // Register HandyPay webhook when a new API key is saved
...
async function handleGet(req, res, locationId) {
  ...
  const pitKey = cfg && cfg.crm_access_token ? mask(cfg.crm_access_token) : '';
  const isPit  = !!(cfg && !cfg.crm_refresh_token && cfg.crm_access_token);
