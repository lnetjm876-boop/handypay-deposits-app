...
// CALENDAR FIX: send custom_element_loaded, check window.name + hash, proxy GHL public order
'use strict';
...
module.exports = async function handler(req, res) {
  ...
  // Build client-side JS as a single clean block — avoids string-termination bugs
  const clientJS = `
...
function dlog(s){dbgLines.push(s);$el('dbg').innerHTML=dbgLines.slice(-10).join('<br>');try{fetch('/api/debug-log',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({label:'pay',data:s})}).catch(function(){});}catch(e){}}
...
function fetchOrder(ordId){
  if(!ordId)return;
  dlog('\u1f50e Fetching order '+ordId.substring(0,12)+'...');
  fetch('/api/ghl-order?orderId='+encodeURIComponent(ordId))
    .then(function(r){return r.json();})
    .then(function(d){if(d.amount>0)applyAmt(d.amount,d.currency,d.description,d.transactionId,d.orderId);else dlog('\u26a0 order amt=0');})
    .catch(function(e){dlog('\u274c order err: '+e.message);});
}
...
function openHP(){
  ...
  .then(function(d){
    if(!d.checkoutUrl){
      dlog('\u274c No checkoutUrl: '+(d.error||'?'));ss('Error: '+(d.error||'No checkout URL'));
  ...
  }).catch(function(e){dlog('\u274c session err: '+e.message);ss('Error: '+e.message);$el('b').disabled=false;done=false;});
}
// INIT
(function init(){
  ...
  dlog((inIframe?'\u2705 iframe':'\u26a0 standalone')+' | '+location.search);
  // 2. window.name
  var winName='';
  try{winName=window.name||'';}catch(e){}
  dlog('window.name: '+(winName?winName.substring(0,60):'(empty)'));
  if(winName){
  ...
  if(hash&&hash.length>1){
    try{
      ...
      if(hOrd)fetchOrder(hOrd);
      else if(hAmt>0)applyAmt(hAmt,hp.get('currency'),'',hp.get('transactionId')||'','');
    }catch(e){}
  }
  // 4. URL params
  if(URL_AMT>0)applyAmt(URL_AMT,URL_CUR,'',URL_TXN,URL_TXN);
  // 5. Timeout warning
  setTimeout(function(){if(!done&&AMT===0)dlog('\u26a0 8s: no amount yet — waiting for GHL postMessage');},8000);
})();
// MESSAGE LISTENER
